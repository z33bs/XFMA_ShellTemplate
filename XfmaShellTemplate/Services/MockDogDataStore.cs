...
namespace XfmaShellTemplate.Services
{
    public class MockDogStore : IDataStore<Dog>
    {
        ...
        {
            var oldItem = dogs.Where((Dog arg) => arg.Id == item.Id).FirstOrDefault();
            dogs.Remove(oldItem);
            dogs.Add(item);
        ...
        {
            var oldItem = dogs.Where((Dog arg) => arg.Id == id).FirstOrDefault();
            dogs.Remove(oldItem);
        ...
        {
            return await Task.FromResult(dogs.FirstOrDefault(s => s.Id == id));
        }
